...
# set container name
my_manim_container_name="my-chemistry-container"

echo -e "${YELLOW} Info: To removed container run: ./run.sh down ${RESET}\n"
echo -e "${YELLOW} Info: To save videos to volume:  run: ./run.sh save ${RESET}\n"
echo -e "${YELLOW} Info: To build MANIM videos ./run.sh  my-python_script.py ${RESET}\n"
echo -e "${YELLOW} Info: To build corresponding docker image:  run: ./run.sh build ${RESET}\n"
...
then 
        # Mount Volume 
        MOUNT_FILES="$(mount -t /dev/disk2s1 && ls  "/Volumes/Disk E"  )"
        if [ "$?" != 0 ];
        then 
            echo -e "${ONCOLOR_FAILED} [ERROR]: Disk E not found ${RESET} \n"
            return 
        fi 
       
        #VOLUME_VIDEOS_PATH="/Volumes/Disk E/VIDEOS-1ere-Maths-mp4/mp4_trigo"
        VOLUME_VIDEOS_PATH="/Volumes/Disk E/VIDEOS-${class_level}-Chemistry-mp4/"
        #MANIM_VIDEOS_BASE_PATH="/Users/whodunit/MY_CHANNELS_MATHS/1ere/Geometry/VIDEO-1ere-PRO-SCALAIRE-1-2-3-4/MANIM/media/videos"
        MANIM_VIDEOS_BASE_PATH="/Users/whodunit/MY_CHANNELS_CHEMISTRY/${class_level}/Organique/Nomenclature/MANIM/media/videos"
